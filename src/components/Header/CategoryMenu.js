import React, { useState } from 'react';
import classes from './CategoryMenu.module.css';

// Default categories
const MENU_BUTTONS = [
	{
		name: 'New & Featured',
		content: [
			{
				group: 'New Arrivals',
				rows: [
					{
						label: 'New Summer Arrivals',
						url: '/#',
					},
					{
						label: 'SNKRS Launch Calendar',
						url: '/#',
					},
					{
						label: 'New & Upcoming Drops',
						url: '/#',
					},
				],
			},
			{
				group: 'Popular Items',
				rows: [
					{
						label: '.Swoosh',
						url: '/#',
					},
					{
						label: 'Jordan Collection',
						url: '/#',
					},
					{
						label: 'Nike SB - The Vault',
						url: '/#',
					},
					{
						label: 'Sustainability',
						url: '/#',
					},
					{
						label: 'Nike Stories',
						url: '/#',
					},
				],
			},
			{
				group: 'Shop Classics',
				rows: [
					{
						label: 'Air Force 1',
						url: '/#',
					},
					{
						label: 'Air Jordan 1',
						url: '/#',
					},
					{
						label: 'Air Max',
						url: '/#',
					},
					{
						label: 'Dunk',
						url: '/#',
					},
					{
						label: 'Blazer',
						url: '/#',
					},
					{
						label: 'Pegasus',
						url: '/#',
					},
					{
						label: 'Mercurial',
						url: '/#',
					},
				],
			},
			{
				group: 'Guides',
				rows: [
					{
						label: 'All Guides',
						url: '/#',
					},
					{
						label: 'Nike Activity Finder',
						url: '/#',
					},
					{
						label: 'Find your size',
						url: '/#',
					},
					{
						label: 'Become a member',
						url: '/#',
					},
				],
			},
		],
	},
	{
		name: 'Men',
		content: [
			{
				group: 'Shoes',
				rows: [
					{
						label: 'All Shoes',
						url: '/#',
					},
					{
						label: 'Lifestyle',
						url: '/#',
					},
					{
						label: 'Jordan',
						url: '/#',
					},
					{
						label: 'Jogging',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Fitness & Training',
						url: '/#',
					},
					{
						label: 'Skateboarding',
						url: '/#',
					},
					{
						label: 'Nike By You',
						url: '/#',
					},
				],
			},
			{
				group: 'Clothing',
				rows: [
					{
						label: 'All Clothing',
						url: '/#',
					},
					{
						label: 'Tops & T-shirts',
						url: '/#',
					},
					{
						label: 'Hoodies & sweatshirts',
						url: '/#',
					},
					{
						label: 'Jackets & Vests',
						url: '/#',
					},
					{
						label: 'Pants & Tights',
						url: '/#',
					},
					{
						label: 'Shorts',
						url: '/#',
					},
					{
						label: 'Underwear',
						url: '/#',
					},
					{
						label: 'Socks',
						url: '/#',
					},
				],
			},
			{
				group: 'Shop By Sport',
				rows: [
					{
						label: 'Golf',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Tennis',
						url: '/#',
					},
					{
						label: 'Baseball',
						url: '/#',
					},
					{
						label: 'Running',
						url: '/#',
					},
					{
						label: 'Training & Gym',
						url: '/#',
					},
					{
						label: 'Cross Country',
						url: '/#',
					},
					{
						label: 'Yoga',
						url: '/#',
					},
					{
						label: 'Pickleball',
						url: '/#',
					},
					{
						label: 'Swim',
						url: '/#',
					},
				],
			},
		],
	},
	{
		name: 'Women',
		content: [
			{
				group: 'Shoes',
				rows: [
					{
						label: 'All Shoes',
						url: '/#',
					},
					{
						label: 'Lifestyle',
						url: '/#',
					},
					{
						label: 'Jordan',
						url: '/#',
					},
					{
						label: 'Jogging',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Fitness & Training',
						url: '/#',
					},
					{
						label: 'Skateboarding',
						url: '/#',
					},
					{
						label: 'Slides & Sandals',
						url: '/#',
					},
				],
			},
			{
				group: 'Clothing',
				rows: [
					{
						label: 'All Clothing',
						url: '/#',
					},
					{
						label: 'Plus Size',
						url: '/#',
					},
					{
						label: 'Bras',
						url: '/#',
					},
					{
						label: 'Tops & T-shirts',
						url: '/#',
					},
					{
						label: 'Hoodies & sweatshirts',
						url: '/#',
					},
					{
						label: 'Jackets & Vests',
						url: '/#',
					},
					{
						label: 'Pants & Tights',
						url: '/#',
					},
					{
						label: 'Shorts',
						url: '/#',
					},
					{
						label: 'Underwear',
						url: '/#',
					},
					{
						label: 'Socks',
						url: '/#',
					},
				],
			},
			{
				group: 'Shop By Sport',
				rows: [
					{
						label: 'Golf',
						url: '/#',
					},
					{
						label: 'Yoga',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Tennis',
						url: '/#',
					},
					{
						label: 'Baseball',
						url: '/#',
					},
					{
						label: 'Running',
						url: '/#',
					},
					{
						label: 'Training & Gym',
						url: '/#',
					},
					{
						label: 'Cross Country',
						url: '/#',
					},
					{
						label: 'Pickleball',
						url: '/#',
					},
					{
						label: 'Swim',
						url: '/#',
					},
				],
			},
		],
	},
	{
		name: 'Kids',
		content: [
			{
				group: 'Shoes',
				rows: [
					{
						label: 'All Shoes',
						url: '/#',
					},
					{
						label: 'Lifestyle',
						url: '/#',
					},
					{
						label: 'Jordan',
						url: '/#',
					},
					{
						label: 'Jogging',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Fitness & Training',
						url: '/#',
					},
					{
						label: 'Skateboarding',
						url: '/#',
					},
					{
						label: 'Nike By You',
						url: '/#',
					},
				],
			},
			{
				group: 'Clothing',
				rows: [
					{
						label: 'All Clothing',
						url: '/#',
					},
					{
						label: 'Tops & T-shirts',
						url: '/#',
					},
					{
						label: 'Hoodies & sweatshirts',
						url: '/#',
					},
					{
						label: 'Jackets & Vests',
						url: '/#',
					},
					{
						label: 'Pants & Tights',
						url: '/#',
					},
					{
						label: 'Shorts',
						url: '/#',
					},
					{
						label: 'Underwear',
						url: '/#',
					},
					{
						label: 'Socks',
						url: '/#',
					},
				],
			},
			{
				group: 'Shop By Sport',
				rows: [
					{
						label: 'Golf',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Tennis',
						url: '/#',
					},
					{
						label: 'Baseball',
						url: '/#',
					},
					{
						label: 'Running',
						url: '/#',
					},
					{
						label: 'Training & Gym',
						url: '/#',
					},
					{
						label: 'Cross Country',
						url: '/#',
					},
					{
						label: 'Yoga',
						url: '/#',
					},
					{
						label: 'Pickleball',
						url: '/#',
					},
					{
						label: 'Swim',
						url: '/#',
					},
				],
			},
		],
	},
	{
		name: 'Accessoires',
		content: [
			{
				group: 'Featured',
				rows: [
					{
						label: 'Trending: Fanny Pack',
						url: '/#',
					},
					{
						label: '$25 & Under',
						url: '/#',
					},
					{
						label: 'Sale',
						url: '/#',
					},
				],
			},
			{
				group: 'All Accessories',
				rows: [
					{
						label: 'Jordan',
						url: '/#',
					},
					{
						label: 'Socks',
						url: '/#',
					},
					{
						label: 'Hats & Headwear',
						url: '/#',
					},
					{
						label: 'Sunglasses & Eyewear',
						url: '/#',
					},
					{
						label: 'Bags & Backpacks',
						url: '/#',
					},
					{
						label: 'Water Bottles & Hydration',
						url: '/#',
					},
					{
						label: 'Gloves',
						url: '/#',
					},
				],
			},
			{
				group: 'Gear Up by Sport',
				rows: [
					{
						label: 'Basketball',
						url: '/#',
					},
					{
						label: 'Golf',
						url: '/#',
					},
					{
						label: 'Football',
						url: '/#',
					},
					{
						label: 'Baseball',
						url: '/#',
					},
					{
						label: 'Track & Field',
						url: '/#',
					},
					{
						label: 'Lacrosse',
						url: '/#',
					},
					{
						label: 'Softball',
						url: '/#',
					},
					{
						label: 'Running',
						url: '/#',
					},
					{
						label: 'Swim',
						url: '/#',
					},
				],
			},
		],
	},
	{
		name: 'Sale',
		content: [
			{
				group: 'All Sale',
				rows: [
					{
						label: 'Sale Shoes',
						url: '/#',
					},
					{
						label: 'Sale Clothing',
						url: '/#',
					},
					{
						label: 'Sale Accessories',
						url: '/#',
					},
				],
			},
			{
				group: 'Men',
				rows: [
					{
						label: 'Shoes',
						url: '/#',
					},
					{
						label: 'Clothing',
						url: '/#',
					},
					{
						label: 'Accessories',
						url: '/#',
					},
					{
						label: 'Shop All',
						url: '/#',
					},
				],
			},
			{
				group: 'Women',
				rows: [
					{
						label: 'Shoes',
						url: '/#',
					},
					{
						label: 'Clothing',
						url: '/#',
					},
					{
						label: 'Accessories',
						url: '/#',
					},
					{
						label: 'Shop All',
						url: '/#',
					},
				],
			},
			{
				group: 'Kids',
				rows: [
					{
						label: 'Shoes',
						url: '/#',
					},
					{
						label: 'Clothing',
						url: '/#',
					},
					{
						label: 'Accessories',
						url: '/#',
					},
					{
						label: 'Shop All',
						url: '/#',
					},
				],
			},
		],
	},
];

const CategoryMenu = (props) => {
	const [activeDropdown, setActiveDropdown] = useState(null);

	const handleMouseEnter = (index) => {
		setActiveDropdown(index);
	};

	const handleMouseLeave = () => {
		setActiveDropdown(null);
	};

	// Return li-element for each category
	const menuButtons = MENU_BUTTONS.map((menuButton, index) => {
		const isDropdownActive = activeDropdown === index;

		return (
			<li
				key={menuButton.name}
				className={`${classes.menuButton} ${isDropdownActive ? classes.open : ''}`}
				onMouseEnter={() => handleMouseEnter(index)}
				onMouseLeave={handleMouseLeave}
			>
				<button>{menuButton.name}</button>
				{isDropdownActive && (
					<div className={classes.dropdownContent}>
						{/* IMPROVE -- MAKE CATEGORYGROUP-COMPONENT */}
						{menuButton.content.map((group, groupIndex) => {
							return (
								<div key={groupIndex} className={classes.categoryGroup}>
									<h2 className={classes['categoryGroup_title']}>{group.group}</h2>
									<div className={classes['categoryGroup_rows']}>
										{group.rows.map((row, rowIndex) => {
											return (
												<div key={rowIndex} className={classes['categoryGroup_row']}>
													<a href={row.url}>{row.label}</a>
												</div>
											);
										})}
									</div>
								</div>
							);
						})}
					</div>
				)}
			</li>
		);
	});

	return <ul className={classes.categoryMenu}>{menuButtons}</ul>;
};

export default CategoryMenu;
